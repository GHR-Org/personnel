...
const getStatusBadgeVariant = (status: string) => {
  ...
  const handleCreate = async (data: PersonnelFormData) => {
    ...
    try {
      ...
      console.log(
        "🔧 Création personnel - Établissement actuel:",
      ...
      console.log(
        "🔧 Création personnel - Données envoyées à l'API:",
        personnelData
      );
      console.log(
        "🔐 Le mot de passe sera automatiquement hashé avant l'envoi au serveur"
      );
  ...
  const handleEdit = async (data: PersonnelFormData) => {
    ...
    try {
      ...
      console.log(
        "🔧 Modification personnel - Personnel à modifier:",
        editingPersonnel
      );
      console.log(
        "🔧 Modification personnel - Statut actuel:",
        editingPersonnel.statut_compte
      );
      console.log(
        "🔧 Modification personnel - Nouveau statut:",
        data.statut_compte
      );
      ...
      console.log(
        "🔧 Modification personnel - Données envoyées à l'API:",
        personnelData
      );
      console.log(
        "🔧 Modification personnel - Statut dans les données envoyées:",
        personnelData.statut_compte
      );
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      // fetchStatistics();
      toast.success(
        `${selectedPersonnel.length} membres du personnel supprimés avec succès`
      );
    } catch (error) {
  ...
  const handleExport = async (format: "csv" | "json") => {
    try {
      ...
      toast.success(
        `Export ${format.toUpperCase()} terminé: ${
          data.length
  ...
  const openViewDialog = (personnel: Personnel) => {
    setEditingPersonnel(personnel);
    setIsEditDialogOpen(true);
  };
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case "Directeur":
        return <Crown className="h-4 w-4 text-indigo-500" />;
      default:
  ...
  if (!currentEtablissement) {
    return (
      <div className="flex items-center min-w-screen justify-center min-h-screen">
  ...
  if (loading) {
    return (
      <div className="flex flex-col p-6 space-y-6 w-screen">
  ...
  return (
    <div className="p-6 space-y-6 w-full relative flex flex-col max-w-none">
      {/* Header */}
      <div className="flex justify-between">
        {/* <Card className="w-full ">
          <CardHeader>
            <CardTitle className="text-lg font-semibold">Personnel de {currentEtablissement.nom}</CardTitle>
        ...
        <div className="flex items-start  flex-col gap-2">
          ...
          <p className="text-muted-foreground text-md">
            Gérez le personnel de votre établissement :{" "}
            <strong>{currentEtablissement.nom}</strong>
          </p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {statistics.total || personnel.length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {
                      personnel.filter((p) => p.statut_compte === "active")
                        .length
                    }
                  </p>
                </div>
                  ...
                  <TableRow key={personnel.id}>
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          handleSelectPersonnel(personnel.id)
                        }
                      />
                    </TableCell>
                    <TableCell>
              ...
              >
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Embauché le{" "}
                    {new Date(personnel.date_embauche).toLocaleDateString(
                      "fr-FR"
                    )}
                  </div>
                </CardContent>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden bg-gradient-to-br from-gray-50 to-white border-2 border-blue-200 shadow-2xl">
          <DialogHeader className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 -m-6 mb-6 rounded-t-lg">
            ...
            <p className="text-blue-100 mt-2">
              Ajoutez un nouveau membre du personnel à votre établissement
            </p>
          </DialogHeader>
