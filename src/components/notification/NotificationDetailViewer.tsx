...
//       case "conge":
//       case "congé":
//         return conges.find((c) => Number(c?.id) === id);
//       case "personnel":
//         return personnels.find((p) => Number(p?.id) === id);
//       case "chambre":
//         return chambres.find((c) => Number(c?.id) === id);
//       case "plat":
//         return plats.find((p) => Number(p?.id) === id);
//       case "produit":
//         return produits.find((p) => Number(p?.id) === id);
//       default:
//         return null;
//     }
...
//   );
// }
export default function NotificationDetailViewer() {
  return(
    <>
    <h1> Pas de notification pour le moment </h1>
    </>
  )
}